...
import { Check, Download, Database, RotateCcw, CheckCircle2, AlertTriangle, XCircle, AlertCircle } from 'lucide-react';
...
export default function NewPredictionPage() {
    ...
    // a file that is uploaded but not yet confirmed shows its preview on the Source step
    const effectiveStep: Step = currentStep === 'upload' && uploadedData ? 'preview' : currentStep;
                            ...
                            <li key={s.key} className="flex shrink-0 flex-col">
                                <div className={`flex items-start gap-3 rounded-xl px-3.5 py-3 ${isActive ? 'border border-blue-200 bg-white' : ''}`}>
                                    ...
                                    <span className="min-w-0">
                                        ...
                                        <span className={`hidden text-xs lg:block ${isActive ? 'text-slate-500' : 'text-slate-400'}`}>{s.hint}</span>
                                    </span>
                                </div>
                        ...
                        <div className="flex flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white">
                            ...
                            <div className="flex flex-col gap-5 px-6 py-5">
                                ...
                                {uploadedData ? (
                                    ...
                                    <div className="flex flex-col gap-4 rounded-xl border border-emerald-200 bg-emerald-50/60 p-5 sm:flex-row sm:items-center">
                                        <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-white text-emerald-700">
                                            <Database className="h-7 w-7" strokeWidth={1.75} />
                                        </div>
                                        <div className="min-w-0 flex-1">
                                            <p className="text-[15px] font-semibold text-slate-900">The history the dashboards already show</p>
                                            <p className="mt-0.5 text-xs text-slate-600">Nothing to upload or map. Continue goes straight to Configure.</p>
                                        </div>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="flex items-center justify-between gap-3 border-t border-slate-100 bg-slate-50/60 px-6 py-4">
                                ...
                                {uploadMode === 'dataset' && !uploadedData ? (
                                    <button type="button" onClick={handleUseDataset} className="rounded-lg bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-700">
            ...
            <div className="overflow-hidden rounded-xl border border-slate-200">
                ...
                <div className="overflow-auto">
                    <table className="w-full text-xs">
                        <thead className="sticky top-0 border-b border-slate-200 bg-slate-50">
                            <tr>
                                ...
                                {d.headers.map((h, i) => <th key={i} className="whitespace-nowrap px-3 py-2 text-left font-semibold text-slate-600">{h}</th>)}
                            </tr>
                        </thead>
                    ...
                    <div className="border-t border-slate-100 bg-slate-50 px-3 py-2 text-center text-[10px] text-slate-400">
                        Showing first 5 of {d.summary.rowCount.toLocaleString()} rows
                    </div>
                </div>
            </div>
        );
