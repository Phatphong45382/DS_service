...
**The scikit-learn framework container is not published in ap-southeast-7 either.** Only the deep-learning containers are, so the first attempt served a 600 KB LightGBM model from the multi-gigabyte `pytorch-inference` image. That endpoint did reach `InService`, but took about twelve minutes to create, and the image it has to pull on every cold start is roughly an order of magnitude larger than the model warrants. Once the endpoint had to move regions anyway, the small `sagemaker-scikit-learn:1.2-1-cpu-py3` container became available, and it is the shape this model needs — a pickled estimator plus a `code/` directory. Only LightGBM installs from `code/requirements.txt` at container start. SHAP was in that list until the endpoint was measured: this container is Python 3.9, `import shap` pulls numba, and numba failed on `numpy._core.multiarray failed to import`, so every request asking for explanations returned 500 while plain predictions worked and the app hid it behind the fallback. LightGBM computes TreeSHAP itself through `predict(pred_contrib=True)` — identical values on this model, base value in the last column — so nothing in the container needs SHAP. That install is not cached between cold starts, which is why `GET /health/warm` exists to pay the cost before anyone is watching.
...
- **The PyTorch deep-learning container** — the only family published in ap-southeast-7, so it was the first attempt, and it does work. Rejected once the endpoint moved regions: it does not fix the serverless gap, and paying a multi-gigabyte image pull on every cold start lands squarely on the one cost ADR-0001 accepts.
